...
class DataAnalysis:
	...
	def make_singlefile_plot(self, raw_audio, filtered_audio, full_audio, sgram_raw, sgram_filtered, \
		...
		text_content = f"Raw Audio: {raw_audio.min():.4f} - {raw_audio.max():.4f}\n" \
						...
						f"Butter: {cnn_config[const.BUTTERPASS_LOW]} - {cnn_config[const.BUTTERPASS_HIGH]} " \
						f"- {cnn_config[const.BUTTERPASS_ORDER]}\n" \
						f"Mel Spectrogram: {sgram_raw.min():.4f} - {sgram_raw.max():.4f}\n" \
						f"File Name: {audio_file_name}"
...
def single():
	analysis = DataAnalysis(const.PHYSIONET_2022)
	analysis.plot_signal_stats(num_samples=3, offset=111, show=True, fig_file_name=None, final_only=True)
