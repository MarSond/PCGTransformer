...
class BEATs(nn.Module):
	def __init__(
		...
		self.logger = logger
		self.logger.info(f"BEATs Config: {cfg.__dict__}")
	...
	def forward_padding_mask(
		...
		self.logger.debug(f"Padding mask shape: {padding_mask.shape}")
		return padding_mask
	...
	def preprocess(
		...
		if source.dim() == 3:  # (batch, 1, samples)
		...
		self.logger.debug(f"FBank min max mean: {fbank.min()} {fbank.max()} {fbank.mean()}")

		fbank = (fbank - fbank_mean) / (2 * fbank_std)
		self.logger.debug(f"final FBank shape: {fbank.shape}\n")
		return fbank

	def extract_features(
		self,
		source: torch.Tensor, # audio signal
		padding_mask: Optional[torch.Tensor] = None,
		fbank_mean: float = 15.41663,
		fbank_std: float = 6.55582,
	):
		self.logger.info(f"extract_features source shape: {source.shape}")
		...
		if self.predictor is not None: # if a pretrained model is used with trained output classes
