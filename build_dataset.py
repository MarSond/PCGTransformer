	...
	for row in name_list.iterrows():
		...
		if name in annotation_classes.index:
			...
			if anno_class == data_c:
				...
				print(	"Name {name} found in annotations and classes is not equal. " \
						f"{anno_class} != {data_c}")
		else:
			print(f"Name {name} not found in regular annotations")
			if name in anno.get("Original record name").values:
				orig_class = (anno.loc[anno["Original record name"] == name].get( \
					"Class (-1=normal 1=abnormal)").values[0])
				data_c = data_classes.loc[name].get("class")
				print(	f"BUT Name {name} found in Original record name with class {orig_class} "
						f"and is in data_classes with class {data_c}")
			else:
...
def get_file_metadata_human_ph2022(path: str, anno: pd.DataFrame, dataset_object: Physionet2022) -> dict:
	...
	patient_id = file_name.split(".")[0].split("_")[0]
	patient_id = int(patient_id)

	# Get the class from the annotation
...
def dataset_statistics(dataset: AudioDataset):
	...
	print("\n")
	print("SR count", train_data[const.META_SAMPLERATE].value_counts())
	print("\n")
	...
	if isinstance(dataset, Physionet2022):
		...
		train_data["cycle_length"] = train_data[const.META_HEARTCYCLES].apply(
			lambda x: sum([end - start for start, end in zip(x[:-1], x[1:])])
		)
		...
		print(
			"Cycle length in seconds / total length in seconds",
			train_data["cycle_length"].sum() / train_data["length_in_seconds"].sum())
		print(
			"Cycle length in samples / total length in samples",
			train_data["cycle_length_in_samples"].sum() / train_data[const.META_LENGTH].sum())
		print(
			"Prozentualer Anteil der Herzzyklen an der Gesamtlänge",
			train_data["cycle_length"].sum() / train_data["length_in_seconds"].sum() * 100,
			"%")
		plt.show()
...
def plot_statistics(dataset: AudioDataset):
	...
	def make_plots(plot_data, dataset: AudioDataset, name=""):
		...
		subaxs[0].pie(
			class_counts,
			labels=[CLASSES_1[0], CLASSES_1[1]],
			autopct="%1.1f%%",
			colors=CLASS_COLORS_1,
		...
		text_content += \
			(f"Standard deviation (Negative): {np.std(seconds[plot_data[CLASS_LABEL_1] == 0]):.2f}s\n")
		text_content += \
			(f"Standard deviation (Positive): {np.std(seconds[plot_data[CLASS_LABEL_1] == 1]):.2f}s\n")
